...
export default function DriverEarningRidelist(props) {
                        ...
                        <View style={{ flexDirection: 'column', flex: 1 }}>
                            <View style={{ flexDirection:'row' }}>
                                ...
                                <View style={{ flex: 1, marginBottom: 10 }}>
                                    <Text style={[styles.textStyle, { marginLeft: 6, textAlign: 'left' }]}>{item.pickup.add} </Text>
                                </View>
                            </View>
                            ...
                            {item && item.waypoints && item.waypoints.length > 0 ?
                                <View style={{ flexDirection:'row' }}>
                                    ...
                                    <View style={{ flex: 1, marginBottom: 10 }}>
                                        <Text style={[styles.textStyle, { marginLeft: 6, textAlign: 'left' }]}>{item.waypoints.length} {('stops')}</Text>
                                    </View>
                                </View>
                                : null}
                            ...
                            <View style={{ flexDirection:'row' }}>
                                ...
                                <View style={{ flex: 1, marginBottom: 10 }}>
                                    <Text style={[styles.textStyle, { marginLeft: 6, textAlign: 'left' }]}>{item.drop.add}</Text>
                                </View>
                            </View>
                        </View>
                    </View>
    ...
    return (
        <View style={styles.textView3}>
            <SegmentedControlTab
                values={[('daily'), ('thismonth'), ('thisyear')]}
                selectedIndex={tabIndex}
                onTabPress={(index) => setTabIndex(index)}
                borderRadius={0}
                tabsContainerStyle={[styles.segmentcontrol, { flexDirection:'row' }]}
                tabStyle={{
                    borderWidth: 0,
                    ...
                    ListEmptyComponent={
                        <View style={{marginTop: height/3.5, justifyContent:'center', alignItems:'center' }}>
                            <View style={{height: 50, minWidth: 150, borderRadius: 10, backgroundColor: MAIN_COLOR, justifyContent:'center', alignItems:'center' }}>
                                <Text style={[styles.textStyleBold,{color: colors.WHITE}]}>{('no_data_available')}</Text>
                            </View>
                        </View>
                    }
                />
